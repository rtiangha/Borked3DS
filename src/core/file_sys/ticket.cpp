...
std::vector<u8> Ticket::Serialize() const {
    ...
    // Add signature type using push_back for each byte
    const u32_be sig_type_be = signature_type;
    const u8* sig_type_bytes = reinterpret_cast<const u8*>(&sig_type_be);
    ret.insert(ret.end(), sig_type_bytes, sig_type_bytes + sizeof(u32_be));

    // Add signature data
    ret.insert(ret.end(), ticket_signature.begin(), ticket_signature.end());

    // Add padding to align to 0x40
    const u32 padding = 0x40 - (ret.size() % 0x40);
    ret.insert(ret.end(), padding, 0);

    // Add ticket body
    const u8* body_data = reinterpret_cast<const u8*>(&ticket_body);
    ret.insert(ret.end(), body_data, body_data + sizeof(ticket_body));
