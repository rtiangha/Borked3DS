...
public:
    SDLGLContext() {
        window = SDL_CreateWindow("", 0, 0, SDL_WINDOW_HIDDEN | SDL_WINDOW_OPENGL);
...
EmuWindow_SDL3_GL::EmuWindow_SDL3_GL(Core::System& system_, bool fullscreen, bool is_secondary)
    ...
    dummy_window = SDL_CreateWindow("", 0, 0, SDL_WINDOW_HIDDEN | SDL_WINDOW_OPENGL);
    ...
    if (!window_context) {
        LOG_CRITICAL(Frontend, "Failed to create GL context: {}", SDL_GetError());
        exit(1);
    }
    if (!core_context) {
        LOG_CRITICAL(Frontend, "Failed to create shared GL context: {}", SDL_GetError());
        exit(1);
    }
    ...
    if (!gl_load_func(reinterpret_cast<GLADloadfunc>(SDL_GL_GetProcAddress))) {
        LOG_CRITICAL(Frontend, "Failed to load GL: {}", SDL_GetError());
        exit(1);
    }
