...
void ConfigureSystem::RefreshSecureDataStatus() {
    ...
    ui->label_secure_info_status->setText(tr(
        (std::string("Status: ") + status_to_str(HW::UniqueData::LoadSecureInfoAFile())).c_str()));
    ui->label_friend_code_seed_status->setText(
        tr((std::string("Status: ") + status_to_str(HW::UniqueData::LoadLocalFriendCodeSeedBFile()))
               .c_str()));
    ui->label_ct_cert_status->setText(tr(
        (std::string("Status: ") + status_to_str(static_cast<HW::UniqueData::SecureDataLoadStatus>(
